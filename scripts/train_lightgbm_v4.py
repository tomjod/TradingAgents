"""
LightGBM Training Script v4 - Regime-Aware Features
Added market regime features and optimized for XAUUSD scalping.

New features:
- Regime classification (TRENDING/RANGING/VOLATILE)
- Peak hours indicator
- Intraday position
- Enhanced H1 features
"""
...
if 'time' in df.columns:
    ...
    # NEW: Peak hours (highest volatility for gold)
    df['peak_hours'] = (
        ((df['hour'] >= 8) & (df['hour'] <= 10)) |  # London Open
        ((df['hour'] >= 13) & (df['hour'] <= 16))    # NY Overlap
    ).astype(int)
    
    # NEW: Off-peak hours (avoid trading)
    df['off_peak'] = ((df['hour'] >= 21) | (df['hour'] <= 6)).astype(int)

# =========================================
# NEW V4 FEATURES: Regime Classification
# =========================================

# ATR-based volatility regime
df['atr_14'] = df['atr'].rolling(14).mean()
df['atr_avg'] = df['atr'].rolling(50).mean()
df['atr_regime_ratio'] = df['atr_14'] / df['atr_avg']

# Classify regime: 0=RANGING, 1=TRENDING, 2=VOLATILE
def classify_regime(row):
    if pd.isna(row['atr_regime_ratio']) or pd.isna(row['adx']):
        return 0
    if row['atr_regime_ratio'] > 1.5:
        return 2  # VOLATILE
    elif row['adx'] > 25:
        return 1  # TRENDING
    else:
...
# One-hot encode regime (better for tree models)
df['regime_trending'] = (df['regime'] == 1).astype(int)
df['regime_volatile'] = (df['regime'] == 2).astype(int)

# =========================================
# NEW V4 FEATURES: Intraday Position
# =========================================

# Where is price in today's range?
df['daily_high'] = df['high'].rolling(288).max()  # 288 = 24h on M5
df['daily_low'] = df['low'].rolling(288).min()
df['intraday_position'] = (df['close'] - df['daily_low']) / (df['daily_high'] - df['daily_low'] + 0.001)
...
if has_h1_data:
    ...
    df_h1['h1_trend'] = np.where(
        ...
        np.where(df_h1['close_10_ema'] > df_h1['close_20_ema'], 2, 1),
        np.where(df_h1['close_10_ema'] < df_h1['close_20_ema'], -2, -1)
    )
    ...
    for col in ['h1_rsi', 'h1_ema_10', 'h1_ema_20', 'h1_atr', 'h1_adx', 'h1_trend']:
        df[col] = df[col].ffill()
    
    # Derived H1 features
    df['h1_rsi_diff'] = df['rsi_14'] - df['h1_rsi']
    df['m5_h1_ema_ratio'] = df['close_10_ema'] / df['h1_ema_10']
    df['atr_ratio_h1'] = df['atr'] / df['h1_atr']
...
if 'hour' in df_clean.columns:
    ...
    features += ['peak_hours', 'off_peak']  # V4 NEW

# Regime features (V4 NEW)
if 'regime_trending' in df_clean.columns:
    features += ['atr_regime_ratio', 'regime_trending', 'regime_volatile']

# Intraday position (V4 NEW)
if 'intraday_position' in df_clean.columns:
...
print(f"\n📊 Total Features (v4): {len(features)}")
print(f"New v4 features: peak_hours, off_peak, regime_*, intraday_position")
...
print(f"\n{'='*50}")
print(f"MODEL PERFORMANCE (v4 - Regime-Aware)")
print(f"{'='*50}")
...
MODEL_PATH = os.path.join(PROJECT_ROOT, "models", "lightgbm_scalper_v4.txt")
best_model.booster_.save_model(MODEL_PATH)
print(f"\n✅ Model saved to {MODEL_PATH}")

PARAMS_PATH = os.path.join(PROJECT_ROOT, "models", "lightgbm_params_v4.json")
with open(PARAMS_PATH, 'w') as f:
    params_to_save = {
        ...
        'new_features': ['peak_hours', 'off_peak', 'regime_trending', 'regime_volatile', 'intraday_position']
    }
    json.dump(params_to_save, f, indent=2)
print(f"Params saved to {PARAMS_PATH}")

print("\n🎉 Training complete! Run backtest to compare v4 vs current model.")
